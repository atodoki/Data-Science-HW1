...
rm(datapoint)
# When inspecting the data, there were 2 main methods used to get info from the datalist
...
for (i in 1:31){
  ...
  datalist[[i]]$Day <- i
  # and let's create some useful factors: age cat, gender, sign_in, day
...
initHistograms <- function(data){
  ...
  hist(as.integer(data$Gender), breaks=2, col = "skyblue", main = "Histogram of Gender")
  hist(data$Impressions, col = "skyblue", main = "Histogram of Impressions")
  hist(data$Clicks, col="skyblue", main = "Histogram of Clicks")
  hist(as.integer(data$Signed_In), breaks = 2, col="skyblue", main = "Histogram of Sign Ins")
  hist(as.integer(data$AgeCat), col = "skyblue", main = "Histogram of Age Group")
}

initHistograms(nytData)

# simple bar graphs, with data from vectors extracted from the lists
clickvector <- c(1:31)
impvector <- c(1:31)
ctrvector <- c(1:31)
dayNames <- c(1:31)
names(clickvector) <- dayNames
names(impvector) <- dayNames
names(ctrvector) <- dayNames
for (i in 1:31){
...
mvector <- c(1:31)
wvector <- c(1:31)
names(mvector) <- dayNames
names(wvector) <- dayNames
for (i in 1:31){
  mvector[i] <- sum(signinlist[[i]]$Gender=="Male")
  wvector[i] <- sum(signinlist[[i]]$Gender=="Female")
}
barplot(mvector, col=c("blue"), main = "Number of (Signed-In) Male Logins Per Day", xlab = "Day")
barplot(wvector, col=c("red"), main = "Number of (Signed-In) Female Logins Per Day", xlab = "Day")
# now with line plots!
